...
class Main extends PluginBase implements Listener{
	...
	private static $MAX_BUILD_HEIGHT = 1000;
				...
				case "/pos1":
					{
						...
						$sender->sendMessage(TextFormat::GREEN . "[MagicWE] Vị trí 1 đã đặt ở x:" . $pos1x . " y:" . $pos1y . " z:" . $pos1z);
						return true;
						break;
					}
				...
				case "/pos2":
					{
						...
						$sender->sendMessage(TextFormat::GREEN . "[MagicWE] Vị trí 2 đã đặt ở x:" . $pos2x . " y:" . $pos2y . " z:" . $pos2z);
						return true;
						break;
					}
				...
				case "/wand":
					{
						...
						if(empty($this->wand[$sender->getName()]) || $this->wand[$sender->getName()] === 0){
							...
							$sender->sendMessage(TextFormat::GREEN . "[MagicWE] Wand đã được bật");
						}
						else{
							$this->wand[$sender->getName()] = 0;
							$sender->sendMessage(TextFormat::RED . "[MagicWE] Wand đã được tắt");
						}
						return true;
						break;
					}
	...
	public function wandPos1(BlockBreakEvent $event){
		...
		if($sender->hasPermission("magicwe.command.wand") && $sender->getInventory()->getItemInHand()->getId() === Item::WOODEN_AXE && $this->wand[$sender->getName()] === 1){
			...
			$sender->sendMessage(TextFormat::GREEN . "[MagicWE] Vị trí đã đặt ở x:" . $block->x . " y:" . $block->y . " z:" . $block->z);
			$event->setCancelled();
		}
	}
	...
	public function wandPos2(PlayerInteractEvent $event){
		...
		if($sender->hasPermission("magicwe.command.wand") && $sender->getInventory()->getItemInHand()->getId() === Item::WOODEN_AXE && $this->wand[$sender->getName()] === 1){
			...
			$sender->sendMessage(TextFormat::GREEN . "[MagicWE] Vị trí 2 đã đặt ở x:" . $block->x . " y:" . $block->y . " z:" . $block->z);
			$event->setCancelled();
		}
	}
	...
	public function fill(Player $player, $blockarg){
		...
		$player->sendMessage(TextFormat::GREEN . "[MagicWE] Đã lấp thành công trong " . round((microtime(TRUE) - $time), 2) . " giây, với " . $changed . " khối đã lấp.");
	}

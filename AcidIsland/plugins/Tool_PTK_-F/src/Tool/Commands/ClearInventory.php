...
class ClearInventory extends BaseCommand{
    ...
    public function __construct(BaseAPI $api){
        parent::__construct($api, "clearinventory", "Xóa sạch kho đồ của bạn hoặc ai đó", "[Tên Người Chơi]", true, ["ci", "clean", "clearinvent"]);
    ...
    public function execute(CommandSender $sender, $alias, array $args): bool{
        ...
        $player->sendMessage(TextFormat::AQUA . "§b→§aKho đôc của bạn đã được xóa!");
        if($player !== $sender){
            $sender->sendMessage("§b→§aKho đồ của người chơi " . TextFormat::AQUA . $player->getDisplayName() . " §ađã được xóa!");
        }
        return true;
    }
}

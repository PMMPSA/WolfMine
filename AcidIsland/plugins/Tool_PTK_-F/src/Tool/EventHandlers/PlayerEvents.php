...
class PlayerEvents extends BaseEventHandler{
    ...
    public function onPlayerPreLogin(PlayerPreLoginEvent $event){
        // Ban remove:
        if($event->getPlayer()->isBanned() && $event->getPlayer()->hasPermission("tool.ban.exempt")){
    ...
    public function onPlayerJoin(PlayerJoinEvent $event){
        ...
        if($i instanceof \DateTime && $event->getPlayer()->hasPermission("tool.mute.notify")){
    ...
    public function onPlayerChat(PlayerChatEvent $event){
        if($this->getAPI()->isMuted($event->getPlayer())){
            if($event->getPlayer()->hasPermission("tool.mute.exempt")){
    ...
    public function onPlayerSleep(PlayerBedEnterEvent $event){
        if($event->getPlayer()->hasPermission("tool.home.bed")){
        ...
        if($victim instanceof Player){
            ...
            if($event instanceof EntityDamageByEntityEvent){
                ...
                if($issuer instanceof Player){
                    ...
                    if($this->getAPI()->isGod($issuer) && !$issuer->hasPermission("tool.god.pvp")){
                        $event->setCancelled(true);
                    }

                    if($this->getAPI()->isVanished($issuer) && !$issuer->hasPermission("tool.vanish.pvp")){
                        $event->setCancelled(true);
                    }
                }
            }
    ...
    public function onPlayerDeath(PlayerDeathEvent $event){
        if($event->getEntity()->hasPermission("tool.back.ondeath")){

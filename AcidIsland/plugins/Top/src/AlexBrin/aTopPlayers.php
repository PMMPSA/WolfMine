	...
	class aTopPlayers extends PluginBase implements Listener {
		...
		public function onEnable() {
			...
			$this->users->exec("CREATE TABLE IF NOT EXISTS users(
														...
														kill INTEGER default 10 NOT NULL,
														death INTEGER default 10 NOT NULL,
														place INTEGER default 100 NOT NULL,
														break INTEGER default 100 NOT NULL
													);
												");
			unset($folder);
		...
		private function create($type) {
			...
			$ah = $this->getServer()->getLevelByName($this->config['coords'][$type]['level']);
			$ah->addParticle($this->particles[$type]);
		}

...
class Main1 extends PluginBase {
    ...
    return $randomString;
	 ...
	 if($cmd->getName() == "gencode-sapling") {
		if($s->isOp()) {
		 $code = $this->generateCode();
		 $s->sendMessage("§l§fNew code§a: ".$code." §b(Sapling Code)");
		}
	 }
	 //}
	 //}
	 if($cmd->getName() == "code-sapling") {
	    if(isset($args[0])) {
		...
		if($this->codeExists($this->giftcode, $args[0])) {
	     ...
	     if(!($this->codeExists($this->used, $args[0]))) {
		   ...
		   $this->getServer()->broadcastMessage("§l§c".$s->getName()."§a was used code§d $args[0] §b(Sapling Code)");
		...
		break;
		  case 40:
			...
			$s->sendMessage("§aYou have received§c 1000 WC");
			$this->coin->addCoin($s->getName(), 1000);
			break;
	       default:
		    $s->sendMessage("§aPlease, check your §finventory");
			$s->sendMessage("§aYou have received§c 1000 WC");

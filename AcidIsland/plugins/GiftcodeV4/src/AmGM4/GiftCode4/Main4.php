...
class Main4 extends PluginBase {
    ...
    return $randomString;
	 ...
	 if($cmd->getName() == "gencode-v4") {
		if($s->hasPermisson("gift.4")){
		 $code = $this->generateCode();
		 $s->sendMessage("§l§bNew code§a: ".$code." ");
		}
	 }else{
	 ...
	 if($cmd->getName() == "code-v4") {
		...
		if($this->codeExists($this->giftcode, $args[0])) {
	     ...
	     if(!($this->codeExists($this->used, $args[0]))) {
		   ...
		   $this->getServer()->broadcastMessage("§l§d".$s->getName()."§a was used code§d $args[0]");
		...
		break;
		  case 40:
			...
			$s->sendMessage("§aYou have received§c 200 Wolfcoin§a with§f 64x diamond");
			$this->coin->addCoin($s->getName(), 200);

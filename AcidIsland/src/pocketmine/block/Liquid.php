...
abstract class Liquid extends Transparent {
           ...
           if($colliding){
           	if($this->getDamage() === 0){
				...
				if($orerand <=5){
                       ...
                       $this->getLevel()->setBlock($this, Block::get(Item::BRICKS_BLOCK), true);
                       }elseif($orerand <=20){
	...
	protected function triggerLavaMixEffects(Vector3 $pos){
		...
		for($i = 0; $i < 8; ++$i){
			$this->getLevel()->addParticle(new FlameParticle($pos->add(mt_rand(0, 80) / 100, 0.5, mt_rand(0, 80) / 100)));
			$this->getLevel()->addParticle(new FlameParticle($pos->add(mt_rand(0, 80) / 100, 0.5, mt_rand(0, 80) / 100)));
			$this->getLevel()->addParticle(new FlameParticle($pos->add(mt_rand(0, 80) / 100, 0.5, mt_rand(0, 80) / 100)));
		}
	}

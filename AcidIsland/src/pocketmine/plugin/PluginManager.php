				...
				foreach(new \RegexIterator(new \DirectoryIterator($directory), $loader->getPluginFilters()) as $file){
					...
					try{
						...
						if($description instanceof PluginDescription){
							...
							$compatibletmntapi = false;
							foreach($description->getCompatibleTmntApis() as $version){
								//Format: majorVersion.minorVersion.patch
								$version = array_map("intval", explode(".", $version));
								$apiVersion = array_map("intval", explode(".", $this->server->getTmntApiVersion()));
							...
							if($compatible === false){
								if($this->server->loadIncompatibleAPI === true){
									$this->server->getLogger()->debug("插件{$name}的API与服务器不符,但TerminatorVN仍然加载了它");
								}else{
							...
							if($compatibletmntapi === false){
								$this->server->getLogger()->error("Could not load plugin '{$description->getName()}': Incompatible TmntAPI version");
								continue;
							}

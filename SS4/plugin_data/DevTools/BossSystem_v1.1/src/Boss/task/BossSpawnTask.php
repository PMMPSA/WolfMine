...
class BossSpawnTask extends Task
{
    ...
    public function onRun(int $currentTick) : void{
        ...
        if(time() >= $plugin->nextSpawn){
            ...
            if($hour <= 24 && $hour !== 0){
                if($min == 60 || $min == 0){
                    $plugin->getServer()->broadcastMessage(TextFormat::AQUA . "§l§2(!)§r §aBoss will spawn in " . $hour . " " . "hour(s)");
                    return;
                }
            }
            ...
            if($remainTime >= 60) {
                switch ($min) {
                    ...
                    case 5;
                    case 1;
                        $plugin->getServer()->broadcastMessage(TextFormat::AQUA . "§l§2(!)§r §aBoss will spawn in " . $min .  "minute(s)");
                        break;
                }
            }else{
                switch($remainTime){
                    ...
                    case 2;
                    case 1;
                    $plugin->getServer()->broadcastMessage(TextFormat::AQUA . "§l§2(!)§r §aBoss will spawn in " . $remainTime . " " . "second(s)");
                    break;
                }
            }

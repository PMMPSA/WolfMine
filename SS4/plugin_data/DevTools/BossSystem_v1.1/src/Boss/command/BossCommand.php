...
class BossCommand extends PluginCommand
{
    ...
    {
    	if(!$sender->hasPermission("boss.boss")){
            $sender->sendMessage("§l§4(!)§r §cYou don't have permision to use this command");
            return false;
        }
        if(!isset($args[0])){
        ...
        switch($args[0]){
            ...
            if(!$sender instanceof Player){
                $sender->sendMessage(TextFormat::RED . "§l§4(!)§r §cThis can be executed only in game");
                return true;
            }

            $bosses = Loader::EGG_LIST;
            if(!in_array($args[1], array_values($bosses))){
                $sender->sendMessage(TextFormat::RED . "§l§4(!)§r §cInvalid boss");
                return true;
            }
            ...
            if(!isset($args[1]) || !isset($args[2])){
                $sender->sendMessage(TextFormat::GREEN . "§l§4(!)§r §c/boss setspawn " . TextFormat::YELLOW . "[amount] [hours|days|minutes|seconds]");
                return true;
            }
            ...
            $sender->sendMessage(TextFormat::GREEN . "§l§4(!)§r §cBoss spawn was set on " . $sender->getX() . ":" . $sender->getY() . ":" . $sender->getZ() . " every " . intval($args[1]) . " " . $args[2]);
            break;
            case "setproperty";

            if(!isset($args[1])){
                $sender->sendMessage(Loader::PREFIX . TextFormat::YELLOW . "§l§4(!)§r §cUsage: /boss setproperty [bossName] [property|propertyGroup] [null|args...] [isBaby=false]");

...
class Main extends PluginBase implements Listener {
	...
	public function onEnable() {
        ...
        $this->tbl = $this->getServer()->getPluginManager()->getPlugin("ThiBangLai");
		...
		Entity::registerEntity(Panda::class, true, ["PandaCar"]);
	}

	public function onTouch(PlayerInteractEvent $event) {
		$item = $event->getItem();
		$player = $event->getPlayer();
		if($item->getCustomName() == "§bXe Máy WolfMine" && $this->tbl->tblXEM($player->getName()) == 1){
		$player->sendPopup("§d•§e Bạn không có bằng lái để sử dụng xe§d •");
		return true;
		}
		if($item->getCustomName() == "§bXe Máy WolfMine" && $this->tbl->tblXEM($player->getName()) == 2){
			$pos = $event->getBlock()->getSide($event->getFace());
			$this->spawnCar($pos, "Car");
			$player->getInventory()->removeItem($player->getInventory()->getItemInHand());
		} elseif($item->getId() == Item::SPAWN_EGG and $item->getDamage() == Entity::PANDA) {
			$pos = $event->getBlock()->getSide($event->getFace());
			$this->spawnCar($pos, "PandaCar");
			$player->getInventory()->removeItem($player->getInventory()->getItemInHand());
		}
	}
	...
	public function onRecive(DataPacketReceiveEvent $event) {
		...
		if($pk instanceof InventoryTransactionPacket) {
			...
			if(!($entity instanceof Car) and !($entity instanceof Panda)) return;
